...
public class TrainLine {
    ...
    public int getSize() {
        return lineMap.length; // change this!
    }
    ...
    public TrainStation travelOneStation(TrainStation current, TrainStation previous) {
        ...
        if (this.findStation(current.getName()) != null){
    ...
    public TrainStation getNext(TrainStation station) {
        if(this.goingRight == true){
            ...
            if(station.equals(this.rightTerminus)){
            ...
            if(station.equals(this.leftTerminus)){
                this.reverseDirection();
                return station.getRight();
            }
            else{
    ...
    public TrainStation findStation(String name) throws StationNotFoundException {
        TrainStation ThisStation = this.leftTerminus;
        while(true){
            if (ThisStation.getName().equals(name)){
                return ThisStation;
            }
            else{
                if(ThisStation.equals(this.rightTerminus)){
    ...
    public void swap(TrainStation This, TrainStation Next){
        ...
        if(Next.getLeft() != null){
            Next.getLeft().setRight(This);
            This.setLeft(Next.getLeft());
        }
        else{
            This.setLeft(null);
        }
    ...
    public void sortLine() {
        ...
        TrainStation This = this.leftTerminus.getRight();
        TrainStation Next;
        this.leftTerminus.setNonTerminal();
        this.rightTerminus.setNonTerminal();
        while(This != null){
            Next = This.getLeft();
            while((Next != null) && (This.getName().substring(0,1).compareTo(Next.getName().substring(0,1)) < 0)){
                swap(This, Next);
                Next = Next.getLeft();

            }

            this.rightTerminus = This;
            This = This.getRight();
        }
        this.rightTerminus.setRightTerminal();


        this.leftTerminus = this.rightTerminus.getLeft().getLeft().getLeft().getLeft();
        this.leftTerminus.setLeftTerminal();
    ...
    public TrainStation[] getLineArray() {
        ...
        int len = 1;
        TrainStation This;
        This = this.leftTerminus;
        while(!This.equals(this.rightTerminus)){
            This = This.getRight();
            len++;
        }

        TrainStation[] NewLine = new TrainStation[len];
        NewLine[0] = this.leftTerminus;
        for(int i = 1; i < len; i++){
            NewLine[i] = NewLine[i-1].getRight();
        }
        return NewLine; // change this
    }
    ...
    public void shuffleLine() {
        ...
        this.leftTerminus.setNonTerminal();
        this.rightTerminus.setNonTerminal();
        TrainStation ThisStation = this.lineMap[0];
        this.leftTerminus = ThisStation;
        ThisStation.setLeftTerminal();
        for(int i=0; i<this.lineMap.length-1; i++){
            ThisStation.setRight(this.lineMap[i+1]);
            ThisStation = this.lineMap[i+1];
        }

        ThisStation.setRight(null);
        ThisStation = this.lineMap[this.lineMap.length-1];
        this.rightTerminus = ThisStation;
        ThisStation.setRightTerminal();
        for(int i=this.lineMap.length-1; i>0; i--){
            ThisStation.setLeft(this.lineMap[i-1]);
            ThisStation = this.lineMap[i-1];
        }

        ThisStation.setLeft(null);
